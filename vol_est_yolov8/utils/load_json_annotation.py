...
class AnnotationLoader:
    def __init__(self, labels:dict, file_names_wo_ending:str, json_path:str, image_path:str):
        '''
        Loads json annotation data

        Args:
        file_names_wo_ending (str): File name without file ending
        json_path (str): Path to json file
        image_path (str): Path to image file
        labels (dict): Dictionary with class labels
            Example: {0: 'dog', 1: 'bicycle', 2: 'umbrella'} (can be extracted from yolo results)
        '''
    ...
    def _extract_data_from_shapes(self, annotation_data):
        ...
        mask_w_label = {key: [] for key in self.labels.values()}
...
class AnnotationConverter(AnnotationLoader):
    ...
    def __init__(self, labels:dict, json_annotation:dict, img:np.ndarray):
