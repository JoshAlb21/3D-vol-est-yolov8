...
def method2_connect_cog(ordered_cogs, random_points, combined_mask, alpha=1, iterations=500, n_samples=100) -> Optional[np.ndarray]:
    ...
    '''
    try:
        orderer = vol_est.extract_skeleton.point_orderer.PointOrderer(ordered_cogs)
    except ValueError:
        return None
    reference_points_extended = np.array(orderer.extended_reference_points)
    refiner = vol_est.extract_skeleton.line_refiner.GraphRefiner(reference_points_extended, random_points, alpha=alpha, iterations=iterations)
    optimized_points = refiner.get_refined_points()
    control_points = refiner.trim_by_mask(combined_mask)
    fitted_points = vol_est.extract_skeleton.line_refiner.sample_points_from_segments(control_points, n=n_samples)
